...
public class MiniMap {
	...
	/**
	 * We initialise and reuse the BufferedImage
	 * here for the whole shizzledizzle
	 * @param parent 
	 * @param handler
	 */
...
class MiniMapRender extends Thread {
		...
		while(parent.getParent().isEnabled) {
			...
			for(int x = -MiniMap.radius; x < MiniMap.radius; x++) {
				for(int z = -MiniMap.radius; z < MiniMap.radius; z++) {
					int y = getHighestBlockY(world, (int)(i+x*zoom), (int)(k+z*zoom));
					
					int py = player.getLocation().getBlockY();
					
					int id = world.getBlockTypeIdAt((int)(x*zoom+i), (int)(y),(int)(z*zoom+k));
					...
					image.setRGB(x+MiniMap.radius, z+MiniMap.radius, color.getRGB());
				}
			}
			...
			image.setRGB(MiniMap.radius+m, MiniMap.radius+n, red.getRGB());
	...
	/**
	 * Another nice convenience method,
	 * I love navigation!
	 * @return MiniMap
	 */
